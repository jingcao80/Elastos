...
ECode NativeProxyBusObject::SetProperty(
    /* [in] */ const String& interfaceName,
    /* [in] */ const String& propertyName,
    /* [in] */ const String& signature,
    /* [in] */ IInterface* value)
{
    Logger::I(TAG, " >> SetProperty: interface: %s, property: %s, signature: %s, value: %s",
        interfaceName.string(), propertyName.string(), signature.string(), TO_CSTR(value));
    ...
    if (!ImplementsInterface(interfaceName.string())) {
        ...
        if (status != E_STATUS_OK) {
            Logger::E(TAG, "SetProperty(): AddInterfaceStatus Exception");
            return status;
        }
    }

    ajn::MsgArg arg;
    status = Org::Alljoyn::Bus::MsgArg::Marshal((Int64)&arg, signature, (PVoid)value);
    if (status == E_STATUS_OK) {
        status = SetProperty(interfaceName.string(), propertyName.string(), arg);
        if (FAILED(status)) {
            Logger::E(TAG, "SetProperty(): ajn::SetProperty Exception");
        }
    }
    else {
        Logger::E(TAG, "SetProperty(): Marshal Exception");
    }

    return status;
}

ECode NativeProxyBusObject::GetProperty(
    /* [in] */ const String& interfaceName,
    /* [in] */ const String& propertyName,
    /* [out] */ IVariant** result)
{
    ...
    */
    ...
    if (!ImplementsInterface(interfaceName.string())) {
        status = AddInterface(pbo, mBusPtr, interfaceName);
        if (status != E_STATUS_OK) {
            Logger::E(TAG, "GetProperty(): AddInterfaceStatus Exception");
            return status;
        }
    }

    ajn::MsgArg value;
    status = GetProperty(interfaceName.string(), propertyName.string(), value);
    if (status == E_STATUS_OK) {
        assert(0 && "TODO");
        // status = Org::Alljoyn::Bus::MsgArg::Unmarshal((Int64)&value, CarDataType_Interface, (PVoid)result);
        if (FAILED(status)) {
            Logger::E(TAG, "GetProperty(): Unmarshal Exception");
        }
    }
    else {
        Logger::E(TAG, "GetProperty(): ajn::GetProperty Exception");
    }
    return status;
}

ECode NativeProxyBusObject::GetAllProperties(
    /* [in] */ const String& interfaceName,
