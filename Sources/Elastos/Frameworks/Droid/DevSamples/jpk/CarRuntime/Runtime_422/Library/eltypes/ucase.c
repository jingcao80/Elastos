...
Char32
ucase_tolower(const UCaseProps *csp, Char32 c) {
...
Char32
ucase_toupper(const UCaseProps *csp, Char32 c) {
    uint16_t props=UTRIE2_GET16(&csp->trie, c);
    if(!PROPS_HAS_EXCEPTION(props)) {
...
int32_t
ucase_getType(const UCaseProps *csp, Char32 c) {
    uint16_t props=UTRIE2_GET16(&csp->trie, c);
