...
root.Elastos = ( function () {
    ...
    _api = require('./elastos.node');

    var _oBridge = _api.require("/data/elastos/TextViewDemo.eco", "CTestEventListener");

    var _oRuntime = _api.require("/data/elastos/JSCarRoot.eco", "CCarRoot");

    var _getModuleInfo = function (asEcoName) {
        _oRuntime.Test_Require_ModuleInfo(asEcoName);
    };

    var _oModule_Elastos_Droid = _getModuleInfo("/system/lib/Elastos.Droid.Core.eco");
    var _oModule_Elastos_Core = _getModuleInfo("/system/lib/Elastos.Core.eco");

    return {
        //oBridge : _oBridge,
        oRuntime : _oRuntime,
        //oCore : _oModule_Elastos_Core,

        Activity : {
            main : _oBridge.GetMainActivity()
        },

        Bridge : {
            init : function () {
                var pEnqueueUIMessage = _oBridge.GetEnqueueUIMessagePtr();
                _api.SetEnqueueUIMessagePtr(pEnqueueUIMessage);

                var iNodeBridge = _api.GetNodeBridge();
                _oBridge.SetNodeBridge(iNodeBridge,1);

                _oBridge.SetActivityListener(jso_activity_cb);
                _oBridge.Unlock();
            }
        },
        ...
        Core : {
            Get : function () {
                return _oModule_Elastos_Core;
            },
            New : function () {
                //TODO:Core_New
            },
            CString : function (asText) {
                var oModule_Elastos_Core = Elastos.oRuntime.Test_Require_ModuleInfo("/system/lib/Elastos.Core.eco");
                return Elastos.Runtime.createObject(oModule_Elastos_Core,"CStringWrapper",asText);
            }
        },

        Droid : {
            New : function (aoClassInfo) {
                //TODO:Droid_New
            }
        },
...
function CString (asText) {
    return Elastos.Core.CString(asText);
}

function Droid_New (aoClassInfo) {
    var oModuleInfo = Elastos.oRuntime.Test_Require_ModuleInfo("/system/lib/Elastos.Droid.Core.eco");
    var args = [oModuleInfo];
...
var jso_activity_cb = (function(oActivity){
