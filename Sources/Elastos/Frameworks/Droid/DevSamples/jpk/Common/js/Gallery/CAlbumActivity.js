...
// #include "src/data/BucketHelper.h"
    var BucketHelper = require("./data/BucketHelper.js")(aoElastos);
...
//             Logger::D(TAG, "onTouch()--ACTION_UP--path:%s", folderPath.string());
                elog(TAG + "onTouch()--ACTION_UP--path:" + folderPath);
//             AutoPtr<IIntent> intent;
                var intent;
//             CIntent::New((IIntent**)&intent);
                intent = Droid_New("Elastos.Droid.Content.CIntent");
...
//             //     i, entry->sourcePath.string(), entry->desc.string(), entry->num.string());
                elog(TAG + "Album "+i+": sourcePath:"+entry.sourcePath+", desc:"+entry.desc+", num:"+entry.num);
//         }
            }
...
//     SetContentView(R::layout::activity_album);
        context.SetContentView(R.layout.activity_album);
...
//             //     i, entry->sourcePath.string(), entry->num.string(), entry->desc.string());
                elog( TAG + " > album "+i+", path:"+entry.sourcePath+", num:"+entry.num+", desc:"+entry.desc);
//         }
            }
...
//     CThread::New(IRunnable::Probe(loadRunnable), (IThread**)&loadThread);
        //loadThread = Droid_New("CThread", loadRunnable);  //TODO
//     assert(loadThread != NULL);
//     loadThread->Start();
        //loadThread.Start();   //TODO

        loadRunnable.Run(); //TODO:to be replaced with new thread
// }
    }
