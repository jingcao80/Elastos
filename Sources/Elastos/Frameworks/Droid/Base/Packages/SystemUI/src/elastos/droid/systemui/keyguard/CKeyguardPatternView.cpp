...
{
    return mLockPatternView->RemoveCallbacks(mCancelPatternRunnable);
}
...
{
    Boolean res;
    if (mLockPatternUtils->CheckPattern(pattern, &res), res) {
        ...
        Int32 attempts;
        mKeyguardUpdateMonitor->GetFailedUnlockAttempts(&attempts);
        if (registeredAttempt &&
...
{
    mHost->mLockPatternView->SetEnabled(TRUE);
    return mHost->DisplayDefaultSecurityMessage();
}
...
CAR_INTERFACE_IMPL(CKeyguardPatternView::MyAnimatorUpdateListener, Object, IAnimatorUpdateListener)
    ...
    /* [in] */ IValueAnimator* animation)
{
    ...
    return mHost->mLockPatternView->Invalidate();
}
...
CAR_INTERFACE_IMPL(CKeyguardPatternView, LinearLayout, IKeyguardSecurityView, IAppearAnimationCreator)
    ...
    /* [in] */ IAttributeSet* attrs)
{
    ...
    mAppearAnimationUtils = new AppearAnimationUtils(context,
            IAppearAnimationUtils::DEFAULT_APPEAR_DURATION, 1.5f /* delayScale */,
    ...
    /* [out] */ Boolean* res)
{
    ...
    Int64 elapsed = SystemClock::ElapsedRealtime() - mLastPokeTime;
    ...
    Boolean tmp;
    mLockPatternView->DispatchTouchEvent(ev, &tmp);
    result = tmp || result;
    ev->OffsetLocation(-left, -top);
    *res = result;
    return NOERROR;
...
ECode CKeyguardPatternView::StartAppearAnimation()
{
    ...
    mAppearAnimationUtils->StartAppearAnimation(states, r, this);

    AutoPtr<ICharSequence> text;
    mHelpMessage->GetText((ICharSequence**)&text);
    if (!TextUtils::IsEmpty(text)) {
        ...
        mAppearAnimationUtils->CreateAnimation(mHelpMessage, 0,
                IAppearAnimationUtils::DEFAULT_APPEAR_DURATION,
                translation,
                interpolator,
                NULL /* finishRunnable */);
    }
...
{
    animatedCell->SetScale(0.0f);
    animatedCell->SetTranslateY(startTranslationY);
    ...
    helper->OfFloat(startTranslationY, 0.0f, (IValueAnimator**)&animator);
    animator->SetInterpolator(interpolator);
    animator->SetDuration(duration);
    animator->SetStartDelay(delay);

    AutoPtr<IAnimatorUpdateListener> lis = new MyAnimatorUpdateListener(this, animatedCell);
    ...
    animator->Start();
    return mLockPatternView->Invalidate();
}
