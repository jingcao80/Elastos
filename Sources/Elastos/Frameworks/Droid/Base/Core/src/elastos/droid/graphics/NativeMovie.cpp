...
void NativeMovie::ensureInfo()
{
    if (fInfo.fDuration == UNINITIALIZED_MSEC && !this->onGetInfo(&fInfo))
        memset(&fInfo, 0, sizeof(fInfo));   // failure
}

SkMSec NativeMovie::duration()
{
    this->ensureInfo();
    return fInfo.fDuration;
}

int NativeMovie::width()
{
    this->ensureInfo();
    return fInfo.fWidth;
}

int NativeMovie::height()
{
    this->ensureInfo();
    return fInfo.fHeight;
}

int NativeMovie::isOpaque()
{
    this->ensureInfo();
    return fInfo.fIsOpaque;
}

bool NativeMovie::setTime(SkMSec time)
{
...
const SkBitmap& NativeMovie::bitmap()
{
...
NativeMovie* NativeMovie::DecodeMemory(const void* data, size_t length) {
    SkMemoryStream stream(data, length, false);
    return NativeMovie::DecodeStream(&stream);
}

NativeMovie* NativeMovie::DecodeFile(const char path[]) {
    std::unique_ptr<SkStreamRewindable> stream = SkStream::MakeFromFile(path);
    return stream ? NativeMovie::DecodeStream(stream.get()) : nullptr;
}
...
class GIFMovie : public NativeMovie
{
public:
...
NativeMovie* NativeMovie::DecodeStream(SkStreamRewindable* stream)
{
