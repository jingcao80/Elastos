...
CARAPI CARObjectAdapter::New(
        ...
        IInterfaceInfo const *interfaceInfo, Local<Object> object) noexcept
{
...
CARObjectAdapter::CARObjectAdapter(IInterfaceInfo const *interfaceInfo, Local<Object> object):
...
static void *_vtable[_MAX_NUM_OF_METHODS];
        ...
        va_list ap) noexcept
{
    ...
    if (!Has(self->_owner->_object, _methodName).FromJust())
        return E_NOT_IMPLEMENTED;

    value = Get(self->_owner->_object, _methodName).ToLocalChecked();
    if (!value->IsFunction())
        return E_NOT_IMPLEMENTED;

    return CARFunctionAdapter::Call(value.As<Function>(), self->_owner->_object, methodInfo, ap);
}
    ...
    _interfaceInfo(interfaceInfo), _owner(owner)
{
    ...
    _methodInfos = __methodInfos, __methodInfos->Release();

    ec = interfaceInfo->GetAllMethodInfos(reinterpret_cast<ArrayOf<IMethodInfo *> *>(_methodInfos.Get()));
    if (FAILED(ec))
        throw Error(Error::TYPE_ELASTOS, ec, "");
}
