...
CarInstanceCobject::CarInstanceCobject(CobjectWrapper* objectWrapper, bool requireAnnotation)
{
    mInstance = objectWrapper;
    mRequireAnnotation = requireAnnotation;
}

void CarInstanceCobject::begin()
{
    // getJNIEnv()->PushLocalFrame(NUM_LOCAL_REFS);
}

void CarInstanceCobject::end()
{
    // getJNIEnv()->PopLocalFrame(0);
}

CarKlass* CarInstanceCobject::getClass() const
{
    if (!mClass) {
        mClass = adoptPtr(new CarClassCobject(mInstance->get(), mRequireAnnotation));
    }
    return mClass.get();
}

IInterface* CarInstanceCobject::carInstance() const
{
    return mInstance->mInstance;
}

void CarInstanceCobject::invokeMethod(const CarMethod* method, CarValue* args, bool& didRaiseUncaughtException)
{
    ...
    if (FAILED(ec)) {
        LOG_ERROR("CarInstanceCobject::invokeMethod unable to get methodInfo of \"%s\"", method->name().utf8().data());
        return;
    }
    ...
    if (numParams > 0) {
        ...
        if (FAILED(ec)) {
           LOG_ERROR("CarInstanceCobject::invokeMethod unable to create argumentList on method %p", method);
           return;
        }
        ...
        for (Int32 i = 0; i < numParams; ++i) {
            ...
            switch (args[i].mIOAttribute) {
                case ParamIOAttribute_In:
                    switch (args[i].mType) {
                        ...
                        default:
                            ALOGD("CarInstanceCobject::invokeMethod ParamIOAttribute_In CarDataType_Other:%d",args[i].mType);
                            break;
                    }
                    break;
                ...
                case ParamIOAttribute_CalleeAllocOut:
                    switch (args[i].mType) {
                        ...
                        case CarDataType_ArrayOf:
                            ...
                            break;
                        default:
                            ALOGD("CarInstanceCobject::invokeMethod ParamIOAttribute_CalleeAllocOut CarDataType_Other:%d", args[i].mType);
                            break;
                    }
                    break;
                        ...
                        case CarDataType_Interface:
                        {
                            ...
                            if (args[i].mObjectWrapper) {
                                if (args[i].mObjectWrapper->getDataTypeInfo()) {
                                    ...
                                    ALOGD("CarInstanceCobject::invokeMethod CarDataType_Interface interfaceInfo is NULL============");
                                }
                            }
                            else {
                                ALOGD("CarInstanceCobject::invokeMethod CarDataType_Interface mObjectWrapper is NULL============");
                            }
                            break;
                        }
                        default:
                            ALOGD("CarInstanceCobject::invokeMethod ParamIOAttribute_CallerAllocOut CarDataType_Other:%d",args[i].mType);
                            break;
                    }
                    break;

                default:
                    LOG_ERROR("CarInstanceCobject::invokeMethod unknown ParamIOAttribute");
                    break;
            }
            ...
            if (FAILED(ec)) {
                ...
                LOG_ERROR("CarInstanceCobject::invokeMethod SetArgument error %s", (const char*)nameBuf);
                return;
            }
    ...
    if(method->isRunOnUiThread()) {
        ...
        if (FAILED(ec)) {
            LOG_ERROR("CarInstanceCobject::invokeMethod invoke failed!");
            return;
        }
    }
...
CarValue CarInstanceCobject::getField(const CarField* field)
{
/*
    //TODO:
