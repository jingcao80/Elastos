...
#include "base/elastos/api/ApplicationStatus_api.h"
...
ApplicationStatusListener::ApplicationStatusListener(
  ...
  DCHECK(sElaApplicationStatusCallback);
  //LLJava_ApplicationStatus_registerThreadSafeNativeApplicationStateListener(
  //LL    base::android::AttachCurrentThread());
  sElaApplicationStatusCallback->elastos_ApplicationStatus_registerThreadSafeNativeApplicationStateListener();
}
...
// static
bool ApplicationStatusListener::RegisterBindings() {
...
static void OnApplicationStateChange(Elastos::Int32 new_state) {
