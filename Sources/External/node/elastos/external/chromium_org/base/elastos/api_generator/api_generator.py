...
nativeMethods_Prototype = []
callbackMethods_Prototye = []
structName = ""
headerFileBaseName = ""

def GetInitCallbackFunction():
    initCallback = "";
    if len(callbackMethods_Prototye) > 0:
        callbackName = "s" + structName 
        initCallback =  "extern \"C\" {\n" \
                        "static struct " + structName + "* " + callbackName + ";\n" \
                        "__attribute__((visibility(\"default\")))\n" \
                        "void Elastos_" + headerFileBaseName + "_InitCallback(Handle32 cb)\n" \
                        "{\n" \
                        "    "  + callbackName + " = (struct " + structName +"*)cb;\n" \
                        "    DLOG(INFO) << \"init pointer for " + callbackName + \
                        " is:\" << " +callbackName + ";\n" \
                        "}\n" \
                        "}; // extern \"C\""
    return initCallback

def GetExportFunctionsDec():
    export_function_dec = ""
    if len(nativeMethods_Prototype) > 0:
        export_function_dec += "#ifdef __cplusplus\n"
        export_function_dec += "extern \"C\"\n"
        export_function_dec += "{\n"
        export_function_dec += "#endif\n"
        for nm in nativeMethods_Prototype:
            export_function_dec += "    extern " + nm + "\n"
        export_function_dec += "#ifdef __cplusplus\n"
        export_function_dec += "}\n"
        export_function_dec += "#endif\n"
    return export_function_dec
def GetCallbackMethodStructDec():
    callbacks_struct = ""
    if len(callbackMethods_Prototye) > 0:            
        callbacks_struct += "struct " + structName + "\n"
        callbacks_struct += "{" + "\n"
        for callback in callbackMethods_Prototye:
            callbacks_struct += "    " + callback + "\n"
        callbacks_struct += "};" + "\n" 
    return callbacks_struct

def GenerateDECContent(input_file):
    file_basename = os.path.basename(input_file).split(".")[0]
    headerMacro = "ELASTOS_" + file_basename.upper() + "_CALLBACK_DEC_HH"
    
    deccontent = "//This file is autogenerated for\n" \
                ...
                "#define " + headerMacro + "\n" \
                "\n\n"
    # ouput the export functions declare
    export_functions = GetExportFunctionsDec()
    if len(export_functions) > 0:
        deccontent += export_functions
        deccontent += "\n\n"
    # ouput the callback struct declare
    callbacks = GetCallbackMethodStructDec()
    if len(callbacks) > 0:
        deccontent += callbacks
        deccontent += "\n\n"
                                             
    deccontent += "#endif //" + headerMacro + "\n"
    #print deccontent
    return deccontent
...
def JavaDataTypeToC(java_type,return_type = False):
  ...
  if return_type :
      string_type = 'Elastos::String'
  else:
      string_type = 'const Elastos::String&'
  java_type_map = {
      'void': 'void',
      'String': string_type,
  ...
  java_array_type_map = {
      ...
      'java/lang/String': 'Elastos::String',
      'java/lang/Class': 'jclass',
      'int': 'Int32',
      'byte': 'Byte',
      'char': 'Char8',
  ...
  if java_type in java_pod_type_map:
    ...
    if java_type[:-2] in java_array_type_map:
      if return_type:
        return "AutoPtr<ArrayOf<" + java_array_type_map[java_type[:-2]] + '> >'
      else:
        return "ArrayOf<" + java_array_type_map[java_type[:-2]] + '>*'
    if java_type[:-2].endswith('[]'):
        if java_type[:-4] in java_array_type_map:
          if return_type:
            return "AutoPtr<ArrayOf<ArrayOf<" + java_array_type_map[java_type[:-4]] + '> > >'
          else:
            return "ArrayOf<ArrayOf<" + java_array_type_map[java_type[:-4]] + '> >*'
    if return_type:
      return 'AutoPtr<ArrayOf<AutoPtr<IInterface> > >'
    else:
      return 'ArrayOf<AutoPtr<IInterface> >*'
...
def MangleCalledByNatives(called_by_natives):
  ...
  for called_by_native in called_by_natives:
    ...
    if method_counts[java_class_name][method_name] > 1:
      # leliang now we don't consider this
      #raise ParseError('leliang not support overloaded function now,fun:%s'%
      #                 method_id_var_name)
...
RE_SCOPED_JNI_RETURN_TYPES = re.compile('jobject|jclass|.*Array')
...
class JNIFromJavaP(object):
  ...
  def __init__(self, contents, options):
    ...
    if not self.namespace:
      self.namespace = 'API_' + self.java_class_name
...
class JNIFromJavaSource(object):
  ...
  def __init__(self, contents, fully_qualified_class, options):
    ...
    # leliang change the priority here, parameter with high priority
    jni_namespace = options.namespace or ExtractJNINamespace(contents)
...
class InlHeaderFileGenerator(object):
  ...
  def __init__(self, namespace, fully_qualified_class, natives,
    ...
    self.header_guard = ('ELASTOS_' + fully_qualified_class.replace('/', '_') + '_JNI').upper()
...
$CALLBACK_STRUCT_DECLARE
$CALLBACK_STRUCT_INIT
...
""")
    values = {
        ...
        'CALLBACK_STRUCT_DECLARE': GetCallbackMethodStructDec(),
        'CALLBACK_STRUCT_INIT': GetInitCallbackFunction()
    }
...
jmethodID g_${JAVA_CLASS}_${METHOD_ID_VAR_NAME} = NULL;""")
  ...
  def GetConstantFieldsString(self):
    if not self.constant_fields:
      return ''
    ret = ['enum Elastos_%s_constant_fields {' % self.class_name]
  ...
  def GetMethodStubsString(self):
    ...
    methodstubs = "\n".join(ret).strip()
    if self.options.native_exports and len(methodstubs) > 0:
      return '\nextern "C" {\n' + methodstubs + '\n};  // extern "C"'
    return methodstubs
...
""")
    signature = 'static bool RegisterNativesImpl('
    if self.init_native:
...
""")
  ...
  def GetJNIFirstParam(self, native, prefix=''):
    ret = []
    if native.static:
            ret = []
    elif native.type == 'method':
        ret = [ prefix +  'IInterface* caller']
    elif native.type == 'function':
            #ret = ['\n    ' + prefix + 'IInterface* jcaller']
            ret = [ prefix +  'IInterface* caller']
    return ret

  def GetParamsInDeclaration(self, native, prefix=''):
    ...
    """
    return ',\n    '.join(self.GetJNIFirstParam(native, prefix) + 
                          [prefix + JavaDataTypeToC(param.datatype) + ' ' + 
                           param.name
...
"""
    if self.options.native_exports:
      template_str += """
__attribute__((visibility("default")))
${RETURN} Elastos_${JAVA_NAME}_native${NAME}(${PARAMS2}) {
    return ${NAME}(${PARAMS_IN_CALL});
}
"""
    template = Template(template_str)
    params_in_call = []
    if not self.options.pure_native_methods and not native.static:
      #params_in_call = ['env', 'jcaller']
      params_in_call = ['caller']
    params_in_call = ', '.join(params_in_call + [p.name for p in native.params])

    # java_name = JniParams.RemapClassName(self.fully_qualified_class)
    # java_name = java_name.replace('_', '_1').replace('/', '_')
    java_name = self.fully_qualified_class.split('/')[-1]
    if native.java_class_name:
      java_name += '_00024' + native.java_class_name
    param2 = self.GetParamsInDeclaration(native, '/* [in] */ ')
    if len(param2) > 0:
        param2 = '\n    ' + param2
    values = {'RETURN': JavaDataTypeToC(native.return_type, True),
              'NAME': native.name,
              'JAVA_NAME': java_name,
              'PARAMS': self.GetParamsInDeclaration(native),
              'PARAMS2': param2,
              'PARAMS_IN_CALL': params_in_call}
    global nativeMethods_Prototype 
    nativeMethods_Prototype += [JavaDataTypeToC(native.return_type) + " " + \
                                "Elastos_" + java_name + "_native" + native.name + \
                                "(" + self.GetParamsInDeclaration(native).replace("\n    ", "") + ");"]
    return template.substitute(values)
  ...
  def GetNativeMethodStubString(self, native):
    ...
    if self.options.native_exports:
      template_str = """\
__attribute__((visibility("default")))
${RETURN} Elastos_${JAVA_NAME}_native${NAME}(
    ${PARAMS_IN_DECLARATION}) {"""
    else:
      template_str = """\
static ${RETURN} ${NAME}(${PARAMS_IN_DECLARATION}) {"""
    template_str += """
...
"""
    ...
    return_type = JavaDataTypeToC(native.return_type, True)
    ...
    if self.options.native_exports:
      ...
      java_name = ''
    param = self.GetParamsInDeclaration(native, '/* [in] */ ')
    ...
    global nativeMethods_Prototype 
    nativeMethods_Prototype += [JavaDataTypeToC(return_type) + " " + \
                                "Elastos_" + java_name + "_native" + native.name + \
                                "(" + self.GetParamsInDeclaration(native).replace("\n    ", "") + ");"]
    return template.substitute(values)
  ...
  def GetCalledByNativeValues(self, called_by_native):
    ...
    if called_by_native.static or called_by_native.is_constructor:
      ...
      first_param_in_declaration_for_api = ''
      first_param_in_call = ('%s_clazz(env)' % java_class)
    else:
      first_param_in_declaration = ', jobject obj'
      first_param_in_declaration_for_api = 'IInterface* obj'
      first_param_in_call = 'obj'
    params_in_declaration = self.GetCalledByNativeParamsInDeclaration(
        called_by_native)
    params_in_declaration_for_api = params_in_declaration.replace("\n   ", "")
    if params_in_declaration:
      params_in_declaration = ', ' + params_in_declaration
    if first_param_in_declaration_for_api and params_in_declaration_for_api:
      params_in_declaration_for_api = ', ' + params_in_declaration_for_api
    ...
    return_type = JavaDataTypeToC(called_by_native.return_type, True)
    ...
    if return_type != 'void':
      ...
      if re.match(RE_SCOPED_JNI_RETURN_TYPES, return_type):
        ...
        #if return_type.endswith('>'):
        #    autoptr_suffix = ' >'
        #return_type = 'AutoPtr<' + return_type + autoptr_suffix
    ...
    return {
        ...
        'GET_METHOD_ID_IMPL': self.GetMethodIDImpl(called_by_native),
        'PARAMS_IN_DECLARATION_FOR_API': params_in_declaration_for_api,
        'FIRST_PARAM_IN_DECLARATION_FOR_API':first_param_in_declaration_for_api
    }
...
${FUNCTION_HEADER}
  ...
  ${RETURN_CLAUSE}
    ...
    callback_prototype_template = Template('''${RETURN_TYPE} (*elastos_${JAVA_CLASS}_${METHOD_ID_VAR_NAME})(\
${FIRST_PARAM_IN_DECLARATION_FOR_API}${PARAMS_IN_DECLARATION_FOR_API});''')
    global callbackMethods_Prototye
    callbackMethods_Prototye += [callback_prototype_template.substitute(values)]
    return ''  # leliang template.substitute(values)
...
def WrapOutput(output):
  ...
  for line in output.splitlines():
    ...
    if len(line) < 120 or line.lstrip()[:1] == '#':
...
def GenerateJNIHeader(input_file, output_apifile, output_decfile, options):
  try:
    if os.path.splitext(input_file)[1] == '.class':
      jni_from_javap = JNIFromJavaP.CreateFromClass(input_file, options)
      content = jni_from_javap.GetContent()
      deccontent = GenerateDECContent(input_file);
    else:
    ...
    sys.exit(1)
  if output_apifile:
    if not os.path.exists(os.path.dirname(os.path.abspath(output_apifile))):
      os.makedirs(os.path.dirname(os.path.abspath(output_apifile)))
    if options.optimize_generation and os.path.exists(output_apifile):
      with file(output_apifile, 'r') as f:
        existing_content = f.read()
        if existing_content == content:
          return
    with file(output_apifile, 'w') as f:
      f.write(content)
  else:
    print output_apifile  # output
  if output_decfile:
    if not os.path.exists(os.path.dirname(os.path.abspath(output_decfile))):
      os.makedirs(os.path.dirname(os.path.abspath(output_decfile)))
    if options.optimize_generation and os.path.exists(output_decfile):
      with file(output_decfile, 'r') as f:
        existing_content = f.read()
        if existing_content == deccontent:
          return
    with file(output_decfile, 'w') as f:
      f.write(deccontent)  
  else:
    print "nativemethods:"
    for method in  nativeMethods_Prototype:
        print method
    print "callbacks:"
    for callback in callbackMethods_Prototye:
        print callback
...
See SampleForTests.java for more details.
  ...
  if options.output_dir:
    ...
    global headerFileBaseName
    headerFileBaseName = root_name
    global structName
    structName = "Ela" + root_name + "Callback"
    output_file = os.path.join(options.output_dir, root_name) + '_api.h'
    output_decfile = os.path.join(options.output_dir, root_name) + '_dec.h'
  if options.jarjar:
    with open(options.jarjar) as f:
      JniParams.SetJarJarMappings(f.read())
  GenerateJNIHeader(input_file, output_file, output_decfile, options)
