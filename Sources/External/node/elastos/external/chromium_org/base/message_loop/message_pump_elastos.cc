...
#include "base/elastos/api/SystemMessageHandler_api.h"
...
//LLusing base::android::ScopedJavaLocalRef;
...
static void DoRunLoopOnce(IInterface* obj, Elastos::Int64 native_delegate,
    Elastos::Int64 delayed_scheduled_time_ticks) {
  ...
  if (!next_delayed_work_time.is_null()) {
    ...
    if (delayed_scheduled_time_ticks == 0 ||
      ...
      //LLJava_SystemMessageHandler_scheduleDelayedWork(env, obj,
      //    next_delayed_work_time.ToInternalValue(),
      //    (next_delayed_work_time -
      //     base::TimeTicks::Now()).InMillisecondsRoundedUp());
      DCHECK(sElaSystemMessageHandlerCallback);
      sElaSystemMessageHandlerCallback->elastos_SystemMessageHandler_scheduleDelayedWork(
              obj, native_delegate, delayed_scheduled_time_ticks);
    }
  }
...
void MessagePumpForUI::Start(Delegate* delegate) {
  ...
  //LLDCHECK(system_message_handler_obj_.is_null());
  DCHECK(!system_message_handler_obj_);

  //JNIEnv* env = base::android::AttachCurrentThread();
  //DCHECK(env);

  //LLsystem_message_handler_obj_.Reset(
  //    Java_SystemMessageHandler_create(
  //        env, reinterpret_cast<intptr_t>(delegate)));
  DCHECK(sElaSystemMessageHandlerCallback);
  system_message_handler_obj_ = sElaSystemMessageHandlerCallback->elastos_SystemMessageHandler_create(
          reinterpret_cast<intptr_t>(delegate));

}

void MessagePumpForUI::Quit() {
  //LLif (!system_message_handler_obj_.is_null()) {
  if (system_message_handler_obj_) {
    //LLJNIEnv* env = base::android::AttachCurrentThread();
    //DCHECK(env);

    //LLJava_SystemMessageHandler_removeAllPendingMessages(env,
    //    system_message_handler_obj_.obj());
    //system_message_handler_obj_.Reset();
    DCHECK(sElaSystemMessageHandlerCallback);
    sElaSystemMessageHandlerCallback->elastos_SystemMessageHandler_removeAllPendingMessages(
            system_message_handler_obj_.Get());
  }
...
void MessagePumpForUI::ScheduleWork() {
  ...
  //    system_message_handler_obj_.obj());
  DCHECK(sElaSystemMessageHandlerCallback);
  sElaSystemMessageHandlerCallback->elastos_SystemMessageHandler_scheduleWork(system_message_handler_obj_.Get());
}
...
void MessagePumpForUI::ScheduleDelayedWork(const TimeTicks& delayed_work_time) {
  ...
  DCHECK(sElaSystemMessageHandlerCallback);
  sElaSystemMessageHandlerCallback->elastos_SystemMessageHandler_scheduleDelayedWork(
          system_message_handler_obj_.Get(), delayed_work_time.ToInternalValue(), millis);
}

// static
bool MessagePumpForUI::RegisterBindings() {
