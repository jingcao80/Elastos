...
{
    if (pDesc->pBlockette) {
        pDesc->pBlockette = (PSTATEDESC)((ptrdiff_t)pDesc->pBlockette + s_nOffset);
        RelocateState(pDesc->pBlockette);
    }
    if (pDesc->pNext) {
        pDesc->pNext = (PSTATEDESC)((ptrdiff_t)pDesc->pNext + s_nOffset);
        RelocateState(pDesc->pNext);
    }
    if (pDesc->pvData) {
        pDesc->pvData = (PVOID)((ptrdiff_t)pDesc->pvData + s_nOffset);
    }
}

void RelocateTemplate(LubeTemplate *pTemplate)
{
    pTemplate->mName = (char *)((ptrdiff_t)pTemplate->mName + s_nOffset);
    if (pTemplate->tRoot.pBlockette) {
        pTemplate->tRoot.pBlockette = (PSTATEDESC)
                    ((ptrdiff_t)pTemplate->tRoot.pBlockette + s_nOffset);
...
void RelocateLube(PLUBEHEADER pLube)
{
    ...
    s_nOffset = (uintptr_t)pLube;

    pLube->ppTemplates = (LubeTemplate **)
                ((ptrdiff_t)pLube->ppTemplates + s_nOffset);
    for (n = 0; n < pLube->cTemplates; n++) {
        pLube->ppTemplates[n] = (LubeTemplate *)
                    ((ptrdiff_t)pLube->ppTemplates[n] + s_nOffset);
...
int LoadLubeFromELF32(const char *pszName, PLUBEHEADER *ppLube)
{
    Elf32_Ehdr      ehdr;
...
int LoadLubeFromELF64(const char *pszName, PLUBEHEADER *ppLube)
{
    Elf64_Ehdr      ehdr;
...
#ifdef _linux
#ifdef _x86
        return LoadLubeFromELF32(NULL, ppLube);
#elif _x64
        return LoadLubeFromELF64(NULL, ppLube);
#endif
#else
...
#ifdef _linux
#ifdef _x86
        return LoadLubeFromELF32(szResult, ppLube);
#elif _x64
        return LoadLubeFromELF64(szResult, ppLube);
#endif
#else
